...
# CAESARS
# NHL
def generate_ceasars_nhl_formatted_events():
    ...
    for event in events:
        ...
        for market in event['markets']:
            ...
            if market_name == 'Moneyline':
                formatted_events[event_tuple] = {market_name: [{'name': convert_team_name_nhl(outcome['name']), 'odds': int(outcome['price']['a'])} for outcome in market['selections']]}
        ...
        for market in res['markets']:
            ...
            if 'Alternative Total Goals' in market['name']:
                ...
                formatted_events[event_tuple][market_name] = [{'name': convert_outcome_name(outcome['name']), 'odds': int(outcome['price']['a'])} for outcome in market['selections']]
            # Spreads
            if 'Alternative Puck Line Handicap' in market['name']:
                ...
                formatted_events[event_tuple][market_name] = [{'name': convert_spread_nhl(outcome['name'], market_name), 'odds': int(outcome['price']['a'])} for outcome in market['selections']]
