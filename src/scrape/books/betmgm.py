import requests
from utils import convert_team_name_nhl, convert_market_name, construct_team_total_market_name, construct_total_market_name, construct_spread_market_name, convert_spread_nhl, convert_outcome_name, generate_event_name
...
def generate_betmgm_nhl_formatted_events():
    ...
    for event in res['widgets'][2]['payload']['items'][0]['activeChildren'][0]['payload']['fixtures']:
        ...
        id_to_tuple[event['id']] = event_name
        for market in event['games']:
            market_name = convert_market_name(market['name']['value']) 
            if market_name == 'Moneyline':
                formatted_events[event_name] = {market_name: [{'name': convert_team_name_nhl(result['name']['value']), 'odds': int(result['americanOdds'])} for result in market['results']]}
    for fixtureId in id_to_tuple:
        ...
        res = requests.get(url, headers=headers).json()
        event_name = id_to_tuple[fixtureId]
        for game in res['fixture']['games']:
            ...
            if 'How many goals will' in game['name']['value'] and '(including overtime and shoot-outs)' in game['name']['value']:
                ...
                formatted_events[event_name][market_name] = [{'name': convert_outcome_name(outcome['totalsPrefix']), 'odds': int(outcome['americanOdds'])} for outcome in game['results']]
            # Totals
            ...
            if game['name']['value'] == 'Spread (including overtime and shoot-outs)':
                ...
                formatted_events[event_name][market_name] = [{'name': convert_spread_nhl(outcome['name']['value']), 'odds': int(outcome['americanOdds'])} for outcome in game['results']]
